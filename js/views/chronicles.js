...
(function (A) {
  ...
  function groupByDay(entries) {
    ...
    // Ordenar días por más reciente primero
    const sorted = Array.from(groups.entries()).sort((a, b) => b[0].localeCompare(a[0]));
    // v1.5.8: ordenar entradas dentro de cada día por ts descendente (más reciente primero)
    return sorted.map(([key, list]) => {
      const sortedList = list.slice().sort((a, b) => (b.ts || 0) - (a.ts || 0));
      return [labelForDay(key), sortedList];
    });
  }
